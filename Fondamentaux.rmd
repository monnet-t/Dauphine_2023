...
**Sinistre sans suite** : il s'agit d'un sinistre n'étant pas couvert par le contrat et n'engendrant pas de dépense pécunière par l'assurance  
...
Cela montre également la limite des données, notamment sur un historique ancien, pourtant nécessaire pour construire un modèle statistique.
...
# A partir de ces données actualisées (malgré un R² de 0,55 sur l'ajustement) nous pouvons nous arrêter sur notre série de point à modéliser
...
Étant donné la matière (les risques climatiques) et la volatilité forte de ce domaine, nous privilégierons une approche GEV.  
...
On notera l'écart entre les lois "usuelles" et la théorie des valeurs extrêmes, particulièrement visible sur les PP observable tous les 200 ou 500 ans. L'approche par loi Beta ou loi Gamma reste très proche, 15 centimes d'écarts sur un coût du risque à 200 ans est assez faible.  
Maintenant, l'approche par les GEV montre un tout autre résultat, avec un ratio de 1.7 environ à 200 ans et supérieur à 2.3 à 500 ans... cela doit être bien maîtrisé pour une intégration dans le tarif.  
Si l'on s'attarde sur les Périodes de Retour calculées pour la PP extrême de 2022, l'écart entre Beta et Gamma et bien plus important (du simple au double) et là encore, la théorie des valeurs extrêmes nous donne à réfléchir... 162 ans contre 2450 ans (Gamma) et 4730 (Beta) on parle de facteur x15 / x30  

Enfin, sous hypothèse de stabilité et que nos données dans le temps soit bien iid (ou encore toutes choses égales par ailleurs) nous pouvons estimer que la probabilité de voir une PP aussi forte sur les 39 dernières années serait de 21.5% avec l'approche par la théorie des valeurs extrêmes, avec les autres lois, cette proba devient 0.8% ou 1.6%, soit très peu probable.  
...
A ce jour, et à ma connaissance, rien n'a été prouvé. Il semble cependant évident que nombre de scientifiques vont s'atteler à ces recherches.


NB : si d'ici la fin de la décennie nous observons à nouveau des pertes aussi fortes, l'approche de la théorie des valeurs extrêmes croisée avec l'approche par la loi binomiale négative (donc d'observer 2 succès sur la période) renvoie une probabilité d'environ 3%... 